    ...
    const mediaBlock = (
        <div className="w-full max-w-80 overflow-hidden rounded-4xl border border-border/60 bg-black shadow-[0_18px_40px_rgba(0,0,0,0.18)]">
            <div className="relative aspect-9/16">
                ...
                <div
                    className={`absolute inset-0 bg-[linear-gradient(180deg,rgba(255,255,255,0.02),rgba(0,0,0,0.54))] transition-opacity duration-300 ${isPlaying && hasLocalVideo ? "opacity-0" : "opacity-100"}`}
                    aria-hidden="true"
                />
                <div
                    className={`absolute inset-x-0 bottom-0 p-5 text-white transition-opacity duration-300 ${isPlaying && hasLocalVideo ? "opacity-0" : "opacity-100"}`}
                >
